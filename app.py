...
def predict_risk(model, scaler, X_row: pd.DataFrame):
    ...
    predicted_class = int(prob_positive >= 0.5)
            ...
            try:
                ...
                if hasattr(model, "predict_proba"):
                    explainer = shap.Explainer(model, scaler.transform(
                        pd.DataFrame([[FEATURE_CONFIG.get(f, {}).get("default", 0)
                    ...
                    st.caption("SHAP explanation for non-sklearn models: adapt this block to your model type.")
                    sv = None

                if sv is not None:
                    sv.feature_names = feature_names
                    fig, ax = plt.subplots(figsize=(6, 4.5))
                    shap.plots.waterfall(sv, show=False, max_display=len(feature_names))
